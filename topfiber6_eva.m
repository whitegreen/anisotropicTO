...
%% MBB-BEAM      topfiber6_eva(95,25,0.4,.3, false)       7.6m*1m  3000*(8.1*5.4)/0.315
              %% topfiber6_eva(120,30,0.4,.3, true)     9.6m*1.2m  3000*(10.2*6.8)/0.35 

function topfiber6_eva( nelx,nely,fibfrac, shearGxy, runTO) 
...
CP=E0*[C11; C21; C31;  C12; C22; C32;  C13; C23; C33]; % shape(9,7), effective elasticity matrix
...
negaLIM = 69.1e6/Ebam; % limit of compressive strain 
strainD1 = zeros(nely+1,nelx); 
for i = 1:nelx
  for j = 1:(nely+1)
      uaxID = -1+ 2*j + 2*(i-1)*(nely+1);   %  2*j + 2*(i-1)*(nely+1);
      ubxID = -1+ 2*j + 2*i*(nely+1);
      tt = (U(ubxID) -  U(uaxID))/0.04; %0.04m is the side length of cell
      if tt>=0 
