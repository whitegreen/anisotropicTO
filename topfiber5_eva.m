%% othotropic topology optimization, adapts from the 88 lines (Andreassen 2011)
% add a breakpoint at last line to see the data in Workspace
% MBB-BEAM   topfiber5_eva(95,25,0.45,.3, true)      7.6m*1m  4000*(8.1*5.4)/0.315     
           % topfiber5_eva(120,30,0.4,.3, true)    9.6m*1.2m  4000*(10.2*6.8)/0.35 

function topfiber5_eva(nelx,nely,fibfrac, shearGxy, runTO) 
rmin =1.5; 
eN = nelx*nely; % number of unit cells
nN = (1+nelx)*(1+nely); % number of nodes

%% MATERIAL PROPERTIES  
Ebam= 9.4e9;
E0= (9/20)* Ebam ; % depth RVE=1.0 or actual depth: 0.007*(9/20)* Ebam; 
smlth=5e-9;  % 
C11=[0  1  0  smlth]; % x1x2, x1, x2, 1
C21=zeros(1,4);  
C31=zeros(1,4);             
C12=zeros(1,4);  
C22=[0  0  1  smlth]; % x1x2, x1, x2, 1
C32=zeros(1,4);             
C13=zeros(1,4);                    
C23=zeros(1,4);  
C33= [shearGxy  0  0 smlth ];   % x1x2, x1, x2, 1  
...
% centralF =  4000*(8.1*5.4)*(0.007/0.315);   % floor load 4kN/m2, RVE depth(0.007), beam depth = 315mm,
centralF =   4000*(10.2*6.8)/0.35;                                 %RVE depth(1.0), beam depth = 350mm,
...
while 500> loop &  change > 0.01   % >0.02  >0.01
    ...
    c_ = (c_./Hs)*Ht;
    l1 = 0; l2 = 10; move = 0.1;   % l2 = 1e9 tricky
    newX=   zeros(2,eN);
...
strainD1 = zeros(nely+1,nelx); 
for i = 1:nelx
  for j = 1:(nely+1)
      uaxID = -1+ 2*j + 2*(i-1)*(nely+1);   %  2*j + 2*(i-1)*(nely+1);
      ubxID = -1+ 2*j + 2*i*(nely+1);
      tt = (U(ubxID) -  U(uaxID))/0.04;
      if tt>=0 
         strainD1(j,i)= tt/posiLIM;
      else
         strainD1(j,i)= tt/negaLIM;
      end
  end
end
strainD2 = zeros( nely,nelx+1);
for i = 1:(nelx+1)
  for j = 1:nely
      uayID =  2*(j+1) + 2*(i-1)*(nely+1);
      ubyID =  2*j + 2*(i-1)*(nely+1); %%%%%
      tt =(U(ubyID) -  U(uayID))/0.04;
      if tt>=0 
         strainD2(j,i)= tt/posiLIM;
      else
         strainD2(j,i)= tt/negaLIM;
      end
  end
end
