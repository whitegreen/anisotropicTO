...
% MBB-BEAM   topfiber5(60,20,0.45,.3)  topfiber5(120,40,0.45,.3)  
% cantilever topfiber5(120,60,0.4,.3)  topfiber5(200,100,0.4,.3) 

function topfiber5(nelx,nely,fibfrac, shearGxy) 
...
CP=[C11; C21; C31;  C12; C22; C32;  C13; C23; C33]; % shape(9,4), effective elasticity matrix (9.4*9/20)*1e6*
...
%% LOADS & SUPPORTS MBB-BEAM, i=1:nelx+1, j=1:nely+1, (i,j) maps to index 2*j + 2*(i-1)*(nely+1)
% F = sparse(2,1,-1,2*nN,1); %  force = -1 on y-axis 
% fixeddofs = union([1:2:(2*nely+1)], [2*nN]); 
% U = zeros(2*nN,1);
